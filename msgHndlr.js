...
module.exports = msgHandler = async (client, message) => {
    try {
        ...
        const adminNumber = ['6281374347075@c.us']
        const isAdmin = adminNumber.includes(sender.id)
        const ownerNumber = '6281374347075@c.us'
...
${desc}`)
    ...
    case '#bikingroup':
            ...
            arg = body.trim().split(' ')
            client.reply(from, mess.wait, id)
            const jhoin = '6281374347075@c.us'
            const gcname = arg[1]
...
YOUTUBE    : https://www.youtube.com/c/JossTV
INSTAGRAM  : https://www.instagram.com/jxxzyshn/
GROUP WA   : https://chat.whatsapp.com/I1OkU7uXLXTCfRgTMpRKu2
WHATSAPP   : https://wa.me/6281374347075\nJangan Lupa bantu subscribe follow ya ${pushname}`, id)
            break    
    ...
    case '#sticker2':
        ...
        case '#loli':
           ...
           await limitAdd(serial)
            ...
            const loli = await axios.get(`https://api.waifu.pics/sfw/waifu`)
            const loly = loli.data
            client.sendFileFromUrl(from, loly.url, 'loli.jpeg', '*LOLI*', id)
            break
        ...
        case '#kpop':
            ...
            break   
             case 'makasih':
                client.reply(from, `*sama-sama bestiee*`, id)
                break
        ...
        case 'shalom':
        case '*Shalom*':
             client.reply(from, `Shalom`, id)
             break    

        case 'absen':
             client.reply(from, `Iya ? berikut link presensi: https://forms.gle/FFYwxCLdGRJCdY1J6\n\nsilahkan isi absensi\nJangan Lupa Join Ya Kak ${pushname}`, id)
             break
        ...
        case '*bor apa kabar?*':
        case '*kk apa kabar?*':
             client.reply(from, `sehat bestie, bestie sehat?`, id)
             break

        case 'kamuudhmakanbelum':
             client.reply(from, `belum bestie, aku ga ada duit nih donasi dong, Ketik *#DONASI* untuk menampilakan menu donasi`, id)
             break

        case 'kamu punya ayang':
             client.reply(from, `punya dong besite:)`, id)
             break

        case 'sayang ga sama aku':
